...
for SYNC_OR_ASYNC in async sync; do
    ...
    new_table
    run "$CFNAME" "$TABLE_PATH" "/mapr/se1/user/vgonzalez/s20150518_100K/0_0_0.csv" "$SYNC_OR_ASYNC"

    echo "1 million rows: $SYNC_OR_ASYNC"
    new_table
    run "$CFNAME" "$TABLE_PATH" "/mapr/se1/user/vgonzalez/s20150518_1M/0_0_0.csv" "$SYNC_OR_ASYNC"

    echo "10 million rows: $SYNC_OR_ASYNC"
    new_table
    run "$CFNAME" "$TABLE_PATH" "/mapr/se1/user/vgonzalez/s20150518_10M/0_0_0.csv" "$SYNC_OR_ASYNC"
done
